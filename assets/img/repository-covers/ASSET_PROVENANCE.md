...
The Projects and Repositories pages use one local 960×540 WebP for each of the 220 public repositories in the synchronized catalog. The same cover manifest powers all English, Simplified Chinese, and Japanese routes.

Final cover sources, after visual review:

- 22 browser captures of verified public demos or project pages.
- 87 repository-native screenshots, figures, generated experiment results, official marks, or diagrams pinned to an exact commit and Git blob.
...
- `SOURCE_AUDIT.json` records the full 189-project default-branch media scan plus eight source-pinned fork audits, with candidate selection, commit/blob pins, upstream attribution, and conservative license-risk notes.
- `GITHUB_OG_SOURCES.json` records the live GraphQL-provided repository preview URL for all 220 repositories.
